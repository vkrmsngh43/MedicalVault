...
public class AccessRequestServiceImpl implements AccessRequestService {
  ...
  public boolean verifyAccessCode(String userId, String requesterId, String accessCode) {
    logger.info(
        "Verifying accessCode for userId: "
            + userId
            + "\n RequestedBy : "
            + requesterId
            + " \n accessCode:"
    ...
    AccessRequestRecords accessRequestRecords =
        accessRequestRecordsRepository.getAccessRequestRecordsByRequesterIdAndUserId(
            Long.parseLong(userId), Long.parseLong(requesterId), accessCode);

    if (accessRequestRecords != null) {
      Date validUntill =
          new Date(
              accessRequestRecords.getAccessCodeGeneratedAt().getTime()
